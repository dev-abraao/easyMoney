...
<div {{ $attributes->merge(['class' => 'flex flex-row justify-between items-center bg-gray-800 p-4 border border-gray-700 rounded-lg hover:bg-gray-750 transition-all duration-200 shadow-sm']) }}>
    <div class="flex flex-col">
        <div class="flex items-center gap-2 mb-2">
            ...
            <p class="font-semibold text-green-400 text-sm uppercase tracking-wide">Payment</p>
        </div>
        <p class="font-medium text-white text-base mb-1 truncate">{{ $payment->description ?? 'Payment received' }}</p>
    ...
    <div class="flex flex-col items-center ml-4">
        ...
        <p class="text-gray-400 text-sm">{{ Carbon\Carbon::parse($payment->date)->format('M d, Y') }}</p>
    </div>
</div>